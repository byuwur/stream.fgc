...
// saveTournamentLocked normalizes, writes, stores, and clones state for mutation methods.
...
func (a *App) RandomizeBracketSeeds() (TournamentState, error) {
	...
	// Empty slots become bracket BYEs, then applyByeAdvancement moves real players forward.
...
func normalizeTournamentState(state TournamentState) TournamentState {
	...
	// Normalize old files at the boundary so the rest of the app can trust the shape.
	...
	if state.Bracket.Matches != nil {
		// Migrate the short-lived legacy location into the canonical top-level map.
...
func ensureBracketSeedAssignments(state *TournamentState) {
	...
	if state.Bracket.Seeds == nil {
		state.Bracket.Seeds = map[string]string{}
		// normalizeBracketSeedAssignments may compress identity mappings back to nil.
		for seed := 1; seed <= state.Event.Size; seed++ {
			key := strconv.Itoa(seed)
			state.Bracket.Seeds[key] = key
		}
...
func normalizeBracketSeedAssignments(state *TournamentState) {
	if state.Bracket.Byes != nil {
		// BYE flags are keyed by bracket seed and must disappear when size shrinks.
	...
	for seed := 1; seed <= state.Event.Size; seed++ {
		...
		if _, ok := state.Players[playerID]; !ok || used[playerID] {
			// Invalid or duplicate player references become empty bracket slots.
			state.Bracket.Seeds[key] = ""
			continue
		}
		used[playerID] = true
	...
	for seed := 1; seed <= state.Event.Size; seed++ {
		...
		// Fill accidental holes with unused player records before treating slots as BYE.
...
func cloneTournamentState(state TournamentState) TournamentState {
	cloned := state
	// Maps are reference types; deep-copy them before returning data to Wails/JS.
...
func resolveParticipant(participant TemplateParticipant, state TournamentState) ResolvedParticipant {
	...
	case "seed":
		// Seed participants resolve through Bracket.Seeds so swaps/randomize do not move players.
	...
	case "winner", "loser":
		// Advancement sources are live match results; pending labels keep unresolved slots visible.
...
func templateFileName(format string, size int) string {
	...
	// Keep common aliases here while the bracket graph itself stays in JSON templates.
	switch {
...
func listISO2SVGCodes(path string) ([]string, error) {
	...
	for _, entry := range entries {
		// Country options are intentionally driven by available SVG filenames.
